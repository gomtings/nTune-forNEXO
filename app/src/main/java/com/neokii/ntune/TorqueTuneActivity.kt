...
class TorqueTuneActivity: BaseTuneActivity() {
    ...
    override fun getItemList(json: JSONObject): ArrayList<TuneItemInfo> {
        ...
        return ArrayList<TuneItemInfo>().apply {
            ...
            add(TuneItemInfo("angle_deadzone_v2", json.getDouble("angle_deadzone_v2").toFloat(), 0.0f, 2.0f, 0.01f, 2))
            add(TuneItemInfo("useSteeringAngle", json.getDouble("useSteeringAngle").toFloat(), 0f, 1.0f, 0f, 0))
        }
    }
}